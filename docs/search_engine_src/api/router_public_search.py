...
# 부록 파일 경로 설정
WWW_PDF_DIR = Path("/home/wizice/regulation/www/static/pdf")
        ...
        with db_manager.get_connection() as conn:
            with conn.cursor() as cur:
                query = """
                    ...
                    FROM wz_appendix
                    WHERE wzruleseq = %s
                      AND COALESCE(wziscurrent, true) = true
                    ORDER BY COALESCE(wzorder, 0), CAST(COALESCE(NULLIF(regexp_replace(wzappendixno, '[^0-9]', '', 'g'), ''), '0') AS INTEGER)
                """
...
async def log_regulation_view(data: Dict[str, Any]):
    ...
    try:
        ...
        # 🆕 데이터 정규화 (중복 저장 방지)
        # rule_name에서 번호 부분 제거 (예: "1.2.3. 명칭" → "명칭")
        import re

        # rule_pubno가 없으면 rule_name에서 추출
        if not rule_pubno and re.match(r'^[\d\.]+\s+', rule_name):
            # rule_name 앞부분이 번호 패턴이면 추출
            match = re.match(r'^([\d\.]+)\s+(.+)$', rule_name)
            if match:
                rule_pubno = match.group(1).rstrip('.') + '.'
                rule_name = match.group(2)

        # rule_name에 여전히 번호가 포함되어 있으면 제거
        if rule_pubno:
            # rule_pubno의 끝 점(.) 제거
            pubno_prefix = rule_pubno.rstrip('.')
            # rule_name이 번호로 시작하면 제거
            if rule_name.startswith(pubno_prefix):
                rule_name = re.sub(r'^' + re.escape(pubno_prefix) + r'[\.\s-–]*', '', rule_name).strip()
