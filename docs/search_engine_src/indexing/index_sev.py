...
JSON_PATH = os.path.join(ROOT_PATH, 'www/static/file')
...
class JsonFileProcessor:
    ...
    def extract_rule_info(self, json_data: Dict, file_info: Dict) -> Dict:
        ...
        # 관련기준 추가 (JCI, 의료기관인증기준 등 검색을 위해)
        related_standards = doc_info.get('관련기준', [])
        if related_standards:
            tags_list.extend(related_standards)
...
def fn_create_article_mapping(index: str) -> bool:
    ...
    mapping = {
        "settings": {
            "number_of_shards": 1,
            "number_of_replicas": 0,
            "index.max_ngram_diff": 8,  # ngram 차이 허용 (max - min)
            "analysis": {
                "tokenizer": {
                    "ngram_tokenizer": {
                        "type": "ngram",
                        "min_gram": 2,  # 한 글자 제외 (최소 2글자부터)
                        "max_gram": 10, # 최대 10글자 (긴 한글 단어 대응)
                        "token_chars": ["letter", "digit"]  # 문자, 숫자만 토큰화
                    }
                },
                "analyzer": {
                    "ngram_analyzer": {
        ...
        "mappings": {
            "properties": {
                ...
                "조문내용": {
                    "type": "text",
                    "analyzer": "ngram_analyzer",        # ngram 분석기 적용 (색인+검색 동일)
                    "fields": {
                        "keyword": {"type": "keyword", "ignore_above": 1024},
                        "standard": {
                            "type": "text",
                            "analyzer": "standard"  # 표준 분석기 필드 (선택적 사용)
                        }
                    }
                },
                "관련이미지": {"type": "nested"},
...
def fn_index_appendix_all(index: str, doc_type: str) -> bool:
    ...
    try:
        ...
        if actions:
            bulk(es, actions)
            logger.info(f"Indexed {len(actions)} appendices")
