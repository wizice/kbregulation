...
def read_json_file(file_path: str) -> Optional[Dict]:
    ...
    try:
        ...
        # www/static/file/로 시작하는 경우 (DB에 저장된 상대 경로)
            ...
            full_path = Path('/home/wizice/regulation') / file_path
        # static/file/로 시작하는 경우
        elif file_path.startswith('static/file/'):
            full_path = Path('/home/wizice/regulation/www') / file_path
        # applib/json/로 시작하는 경우
        elif file_path.startswith('applib/json/'):
            full_path = Path('/home/wizice/regulation/fastapi') / file_path
        # 기타 상대 경로는 여러 위치 시도
        else:
            # 1. www/static/file 경로 시도
            full_path = Path('/home/wizice/regulation/www/static/file') / file_path
            if not full_path.exists():
                # 2. fastapi/applib 경로 시도
                full_path = Path('/home/wizice/regulation/fastapi/applib') / file_path
