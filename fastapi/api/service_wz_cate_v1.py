...
"""
    ...
    WZ_CATE table API service router v1
...
from .query_wz_cate_v1 import WzCateTable
from app_logger import setup_logging, get_logger

logger = get_logger(__name__)

# Pydantic Models for Request/Response
class WzCateBase(BaseModel):
    account_seq: int
    device_seq: int
    timestamp: datetime
...
class WzCateCreate(WzCateBase):
    pass

class WzCateUpdate(BaseModel):
    dg: Optional[float] = None
    dn: Optional[float] = None

class WzCateResponse(WzCateBase):
    id: int
...
# API Router 생성
router = APIRouter(
    prefix="/WZ_CATE/api/v1",
    tags=["wz_cate_v1"],
    responses={404: {"description": "Not found"}},
)

# 의존성: WzCateTable 인스턴스 생성
def get_wz_cate_table():
    """WzCateTable 인스턴스를 생성하는 의존성"""
...
async def select_data(
    ...
    wz_cate: WzCateTable = Depends(get_wz_cate_table)
):
    """데이터 조회"""
    try:
        wz_cate.connect()
...
async def insert_data(
    ...
    try:
        ...
        result_id = wz_cate.save(insert_data)
        
        # 삽입된 데이터 조회
        new_record = wz_cate.get_by_id(result_id) if result_id else None
...
async def update_data(
    ...
    try:
        ...
        if update_dict:
            result = wz_cate.update_device_data(id, **update_dict)
        else:
            result = False
            
        # 업데이트된 데이터 조회
        updated_record = wz_cate.get_by_id(id) if result else None
...
async def delete_data(
    ...
    try:
        ...
        # 삭제 실행
        result = wz_cate.delete(wz_cate.TABLE_NAME, {"id": id}, usecon=True)
...
async def save_data(
    ...
    try:
        ...
        result_id = wz_cate.save(data)
        
        # 저장된 데이터 조회
        saved_record = wz_cate.get_by_id(result_id) if result_id else None
...
async def download_excel(
    ...
    try:
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            df.to_excel(writer, sheet_name='WzCate Data', index=False)
        
        output.seek(0)
        
        # 파일명 생성
        filename = f"wz_cate_data_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
...
async def upload_excel(
    ...
    try:
        ...
        # 일괄 삽입
        wz_cate.connect()
        inserted_count = wz_cate.bulk_insert(data_list)
        wz_cate.close()
...
async def get_aggregated_data(
    ...
    try:
        ...
        data = wz_cate.get_aggregated_data(
...
async def delete_old_data(
    ...
    try:
        wz_cate.connect()
        deleted_count = wz_cate.delete_old_data(days=request.days)
        wz_cate.close()
...
async def get_device_list(
    ...
    try:
        wz_cate.connect()
        devices = wz_cate.get_device_list_by_account(account_seq)
        wz_cate.close()
...
# 헬스체크
@router.get("/health")
async def health_check(wz_cate: WzCateTable = Depends(get_wz_cate_table)):
    """API 헬스체크"""
    try:
        con     = wz_cate.connect()
        # 간단한 쿼리로 DB 연결 확인
        result = wz_cate.query("SELECT 1", con=con)
        wz_cate.close()
